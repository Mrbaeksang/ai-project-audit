...
> AI에게 이 파일을 통째로 던지고, 아래 서비스 프로필을 채워서 같이 전달하면 됩니다.
...
1단계: 이 프로필 기반으로 🔴 필수 항목과 🟠 권장 항목 목록을 먼저 알려줘.
2단계: [프레임워크명]으로 기능 코드 전에 저 항목들의 골조만 구현해줘.
       기능은 아직 짜지 말고.
```
...
마지막에 ❌ 항목들을 위험도 순으로 정렬해서 "이것부터 고쳐라" 리스트 줘.
```

### 특정 섹션만 집중 리팩토링
```
AUDIT_SPEC.md 섹션 [번호]([영역명])만 기준으로
...
### 신규 기능 추가 전 체크
```
새 기능 [기능명]을 추가하기 전에 AUDIT_SPEC.md 기준으로
이 기능 구현 시 놓치기 쉬운 체크포인트만 골라서 알려줘.
(운영 안정성, 보안, 데이터 무결성 중심으로)
```

...
>
> 이 README 하나만 AI에 통째로 던지고, 아래 서비스 프로필을 채워서 같이 전달하면 됩니다.
> AI는 프로필 기반으로 각 항목의 필요 여부를 판단하고, 코드를 점검합니다.

---

## 📚 목차

1. [서비스 프로필 (먼저 채워라)](#-서비스-프로필-먼저-채워라)
2. [등급 기준](#-등급-기준)
3. [언제 이 스펙을 던질지 (단계별 사용)](#-언제-이-스펙을-던질지-단계별-사용)
4. [0. 프로젝트 골조 (기능 코드 전에 확정)](#0-프로젝트-골조-기능-코드-전에-확정)
5. [1. 운영 안정성](#1-운영-안정성-operational-reliability)
6. [2. 보안](#2-보안-security--owasp-top-10-기반)
7. [3. 코드 아키텍처](#3-코드-아키텍처-code-architecture)
8. [4. 데이터 무결성](#4-데이터-무결성-data-integrity)
9. [5. 성능](#5-성능-performance)
10. [6. 테스트](#6-테스트-testing)
11. [7. DevOps / 배포](#7-devops--배포)
12. [8. 확장성](#8-확장성-scalability)
13. [9. 12-Factor App](#9-12-factor-app)
14. [10. 문서화](#10-문서화-documentation)
15. [AI 프롬프트 템플릿](#-ai-프롬프트-템플릿)
...
> 각 항목 옆 등급 = "이 조건이면 반드시 필요".
> AI에게: 서비스 프로필 기반으로 각 항목이 이 서비스에 해당하는지 먼저 판단한 뒤 점검하세요.
> 건너뜀(⏭️) 처리 시 반드시 이유를 한 줄로 명시하세요.
...
| 단계 | 던질 범위 | 목적 |
|------|-----------|------|
| **[1] 프로젝트 시작 직전** | 섹션 **0** + 서비스 프로필만 | 골조/규칙부터 확정. 기능 코드 전에 폴더 구조·네이밍·Git 규칙·pre-commit 잡기 |
| **[2] 기능 1~2개 구현 후** | 섹션 0, 1, 2, 6만 (🔴🟠 등급만) | 스케일 전에 **안 빠뜨려야 할 최소치** 확인 |
| **[3] 출시 전** | 전 섹션 | 프로덕션 기준으로 전면 감사 |
| **[4] 이슈 생겼을 때** | 해당 영역 섹션만 | 문제 영역 집중 점검 |
| **[5] 신기능 추가 전** | 해당 영역 + 섹션 1, 2, 4 | 기능이 놓치기 쉬운 횡단 관심사 체크 |

---

## 0. 프로젝트 골조 (기능 코드 전에 확정)

> **언제 쓰나**: 프로젝트 시작 직후, **기능 코드 한 줄 짜기 전에**.
> 여기 빠뜨리면 나중에 "폴더 구조 갈아엎기"·"네이밍 대공사"·"Git 히스토리 오염" 등 회복 불가능한 기술부채가 생깁니다.
...
```
README.md   ← 이 파일 하나로 전부 (서비스 프로필 + 체크리스트 + AI 프롬프트)
```
